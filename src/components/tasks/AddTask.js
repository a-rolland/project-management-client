...
class AddTask extends Component {
  ...
  handleFormSubmit = (event) => {
    ...
    axios.post("http://localhost:3001/api/tasks", { title, description, projectID })
    .then( () => {
  ...
  render(){
    return(
      <div>
            <hr />
            <button className="btn btn-secondary" onClick={() => this.toggleForm()}> Add task </button>
